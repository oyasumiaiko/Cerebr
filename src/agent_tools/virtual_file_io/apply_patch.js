...
export function buildVirtualFileApplyPatchFunctionToolDefinition() {
  return {
    ...
    description: '对可写虚拟纯文本文件应用 Codex apply_patch，可修改 Markdown、HTML、JS、CSS 等文本内容。默认修改 workspace 可写区；skill 文件需传 `target.kind="skill"` 与 `target.name`。本地映射路径 `local/...` 是只读的，需要修改时先用 copy_file 复制到 workspace。如果在 workspace 中新增或修改了需要交付给用户的文件，最终回复应给出 Markdown 相对路径链接，例如 [计划](workspace/plan.md)。如果交付的是可视化页面，请创建 `workspace/*.html` 并链接它，例如 [预览页面](workspace/preview.html)，Cerebr 文件展示框会直接用 sandbox iframe 渲染该 HTML。',
    strict: false,
    parameters: {
      type: 'object',

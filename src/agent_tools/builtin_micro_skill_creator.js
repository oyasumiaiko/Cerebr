...
function buildSkillCreatorInstruction() {
  return [
    ...
    '2. 渐进式披露：默认上下文只看 summary；先用 `list_files` / `search_files` 找目标，再 `read_detail` 或 `read_file` 精读；只有确实需要整包上下文时才 `read_package`。',
    ...
    '5. 文件编辑统一走 `apply_patch`：如果只改一部分逻辑，优先 `search_files` + `read_file` + `apply_patch` 精确修改该文件，并尽量增量更新，不要整文件重写。',
    ...
    '',
    '## 推荐工作流',
    '1. 先 `list` 看是否已有类似 skill；如果是更新，先 `list_files` / `search_files` 找目标，再 `read_detail` + 必要的 `read_file` 了解现状。',
    ...
    '3. 若只是修改局部逻辑，优先 `search_files` -> `read_file(include_line_numbers=true)` -> `apply_patch`。',
    '4. 需要新增文件、修改已有文件，或调整 `manifest.json` 时，统一继续使用 `apply_patch`；只有整 skill metadata 与多文件整体重构时再考虑 `update`。',
...
export function buildBuiltinSkillCreatorRecord() {
  return {
    ...
    interface: {
      ...
      default_prompt: 'When asked to create or update a Cerebr micro skill, first read the built-in skill-creator detail, then inspect the target skill and edit only the necessary files.'
    },
    match: ['<all_urls>'],
    enabled: true,
    instruction: {

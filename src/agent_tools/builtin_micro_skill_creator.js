...
function buildSkillCreatorInstruction() {
  return [
    ...
    '6. 保持低耦合：`SKILL.md` 负责工作流与触发说明；`references/` 负责按需加载的细节；`scripts/` 负责可执行辅助脚本；`assets/` 负责输出素材。',
    '',
    '## 推荐工作流',
    ...
    '4. 如果需要资源文件，再对 `scripts/`、`references/`、`assets/` 下的示例文件做增量修改或删除。',
    '5. 只有当 skill 真的需要页面 runtime 时，才继续 patch `manifest.json` 增加 `match` 与 `runtime.entry_path`，并新增 JS 文件。',
    '6. 是否启用与挂载放在最后：准备好后再 `enable_skill`；只有它已经成为页面 runtime skill 时，才 `mount_on_current_page`。',
    '',
    '## create_skill 模板入口参数',
    ...
    '- 读取顺序：什么时候先看 `SKILL.md`，什么时候再读 `references/` 或执行 `scripts/`。',
    '- 风险与边界：哪些前置条件、限制、常见错误或副作用必须先说明。',
    '- 如果后来加入 runtime：在 `SKILL.md` 里补一小段 runtime notes，说明有哪些 method、典型输入是什么、什么时候可以在页面里调用它们。',
    '',
    '## `SKILL.md` 不应该写什么',
    '- 大段复制外部文档。',
    '- 与具体源码重复的大块实现细节。',
    ...
    '- 这时才需要在 `SKILL.md` 里补充 runtime notes，例如推荐使用 `$invoke("<skill-name>", "methodName", args)` 之类的调用方式。',
    '',
    '## 常见错误',

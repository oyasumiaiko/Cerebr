...
function buildSkillCreatorInstruction() {
  return [
    ...
    '4. 文件工具按协议分工：读取、搜索、复制和移动 skill 文件时，用 `list_files` / `search_files` / `read_file` / `copy_file` / `move_file`，并传 `target: { kind: "skill", name: "<skill-name>" }`；修改、新建或删除文件时使用官方 `apply_patch`，把 operation.path 显式写成 `@skill/<skill-key>/<relative-path>`。',
    '5. `apply_patch` 不是自定义 function：它没有 `target`、`patch` 等自定义参数，只使用 Responses API 返回的 `apply_patch_call.operation`；删除文件使用其中的 `delete_file` operation，不存在单独的顶层 `delete_file` 工具。',
    '6. runtime 是后续演进，不是默认前提：只有当 skill 真的需要在页面里挂载 JS 能力时，才给它补 `manifest.json` 的 `match`、`runtime.entry_path` 和对应 JS 文件。',
    '7. 当前浏览器里模型唯一可直接执行代码的路径是 `js_runtime_execute`：它的 `code` 会作为 async 函数体执行，可直接写 `await` / `return` / `console.log`。',
    '8. 保持低耦合：`SKILL.md` 负责工作流与触发说明；`references/` 负责按需加载的细节；`scripts/` 负责可复用的 JS 片段或待迁移到 runtime 的代码草稿；`assets/` 负责输出素材。',
    '',
    '## 推荐工作流',
    '1. 如果是更新已有 skill，先 `skill_registry(action="list", include_all_sites=true)` 找到目标，再 `read_file(target={kind:"skill",name}, path="SKILL.md")` 读说明入口。',
    '2. 如果是新建 skill，先调用 `skill_registry(action="create_skill", skill={ name, description, interface?, resources?, examples? })` 生成官方风格通用骨架。',
    '3. scaffold 创建完成后，用官方 `apply_patch` 更新 `@skill/<skill-key>/SKILL.md`，把 `Overview`、`Structuring This Skill`、主章节占位符和 `Resources (optional)` 改成真实内容。',
    '4. 如果需要资源文件，再用 `@skill/<skill-key>/scripts/...`、`@skill/<skill-key>/references/...`、`@skill/<skill-key>/assets/...` 路径增量创建、修改或删除；删除使用 `delete_file` operation。不要把 `scripts/` 里的文件当成 shell/python 可直接执行命令。',
    '5. 只有当 skill 真的需要页面 runtime 时，才继续更新 `@skill/<skill-key>/manifest.json` 的 `match` 与 `runtime.entry_path`，并通过 `create_file` operation 新增 JS 文件。',
    ...
    '- 添加 runtime 时，再 patch `@skill/<skill-key>/manifest.json` 增加 `match` 与 `runtime.entry_path`，并新增对应 JS 文件。',
    ...
    '1. 先 `read_file(target={kind:"skill",name:"skill-creator"}, path="SKILL.md")`，确认工作流和字段职责。',
    '2. 如果是修改已有 skill，先 `skill_registry(action="list", include_all_sites=true)` 找到目标，再读它的 `SKILL.md`，必要时继续读相关文件。',
    '3. 若是新建 skill，先调用 `skill_registry(action="create_skill", skill={ name, description, interface?, resources?, examples? })` 生成通用模板。',
    '4. 模板生成后，按返回的 `next_steps` 继续：优先 patch `@skill/<skill-key>/SKILL.md`，再处理资源文件；如果以后真的需要 runtime，再补对应 skill 路径下的 `manifest.json` 与 JS 文件。',
    '5. 若只是修改局部逻辑，优先 `search_files(target={kind:"skill",name}, pattern="...", glob="...")` -> `read_file(target={kind:"skill",name}, path="...", numbered=true)` -> 官方 `apply_patch`，并把 operation.path 写成 `@skill/<skill-key>/<relative-path>`；复制和重命名仍用 `copy_file` / `move_file`。',
    '6. 删除单个 skill 文件时使用 `apply_patch_call.operation.type="delete_file"` 和完整 `@skill/...` 路径；删除整个 skill package 才使用 `skill_registry(action="delete_skill", ...)`。',
    '7. skill lifecycle 动作用 `skill_registry`；文件编辑继续使用文件工具，不要把文件改动塞回 registry action。',
    '',
    '## 判断一个 skill 是否“做对了”',

...
function buildSkillCreatorInstruction() {
  return [
    ...
    '4. 文件编辑统一走顶层文件工具：对 skill 文件的读取、搜索和修改都用 `list_files` / `search_files` / `read_file` / `copy_file` / `move_file` / `delete_file` / `apply_patch`，并传 `target: { kind: "skill", name: "<skill-name>" }`。',
    ...
    '5. 若只是修改局部逻辑，优先 `search_files(target={kind:"skill",name}, pattern="...", glob="...")` -> `read_file(target={kind:"skill",name}, path="...", numbered=true)` -> `apply_patch(target={kind:"skill",name}, ...)`；复制、重命名、删除文件时用 `copy_file` / `move_file` / `delete_file`。',

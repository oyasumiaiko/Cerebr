...
function buildSkillCreatorInstruction() {
  return [
    ...
    '5. 若只是修改局部逻辑，优先 `search_files(target={kind:"skill",name}, pattern="...", glob="...")` -> `read_file(target={kind:"skill",name}, path="...", numbered=true)` -> `apply_patch(target={kind:"skill",name}, ...)`。',

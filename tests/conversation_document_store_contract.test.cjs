...
test('indexeddb_helper 使用 v7 无全文复制迁移并在删除会话时级联清理', async () => {
  const source = await readWorkspaceFile('src/storage/indexeddb_helper.js');

  assert.match(source, /const CHAT_HISTORY_DB_VERSION = 7;/);
  ...
  assert.match(source, /db\.deleteObjectStore\('conversation_metadata'\)/);
  assert.match(source, /db\.deleteObjectStore\('conversation_search'\)/);
  ...
  assert.match(source, /db\.transaction\(\['conversations', CONVERSATION_DOCUMENT_STORE, LOCAL_FILE_MOUNT_STORE\], 'readwrite'\)/);

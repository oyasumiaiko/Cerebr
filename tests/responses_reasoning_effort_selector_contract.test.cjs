...
test('Responses 推理强度上拉菜单固定在输入框与文件按钮之间', async () => {
  ...
  assert.match(sidebarHtml, /id="reasoning-effort-button"[\s\S]*far fa-signal-bars/);
  assert.doesNotMatch(sidebarHtml, /fa-brain/);
  assert.doesNotMatch(sidebarHtml, /fa-microchip-ai/);
  assert.doesNotMatch(sidebarHtml, /fa-gauge-simple/);
...
test('设置默认显示四档且快捷菜单从官方全集动态组装可见档位', async () => {
  ...
  assert.match(settingsSource, /getResponsesReasoningEffortMenuOptions[\s\S]*\['default', \.\.\.\(services\.apiManager\?\.getResponsesReasoningEffortOptions/);
  assert.match(settingsSource, /key: 'responsesReasoningEffortSliderOptions'[\s\S]*type: 'multi_select_dropdown'[\s\S]*label: 'Responses 推理强度快捷菜单'[\s\S]*options: getResponsesReasoningEffortMenuOptions/);
  assert.match(settingsSource, /placeholder: '选择快捷菜单显示的推理强度'/);
  assert.match(settingsSource, /normalizeResponsesReasoningEffortSliderOptions[\s\S]*getResponsesReasoningEffortMenuOptions\(\)\.filter/);
  assert.match(sidebarEventsSource, /reasoningEffortOptions = \['default', \.\.\.officialOptions\]\.filter\(\(effort\) => configuredSet\.has\(effort\)\)/);
  ...
  assert.match(sidebarEventsSource, /effort !== 'default'[\s\S]*reasoning-effort-divider[\s\S]*role', 'separator'/);
